...
async function gatherInventoryBtnClicked(inProgress) {
    ...
    let progressInterval = setInterval(async () => {
        ...
        if (pollResults && 'total' in pollResults && 'passed' in pollResults) {
            const passPercent = (pollResults.total > 0) ? Math.round((pollResults.passed / pollResults.total) * 100) : 0;
            progressText.textContent = `Passed ${pollResults.passed} of ${pollResults.total} tests ${passPercent}% ...`;
            progressBar.value = passPercent;
        }
        if (pollResults.finished) {
            clearInterval(progressInterval);
            progressText.textContent = progressText.textContent.replace('...', '') + ", DONE";
        }
        else if (!('total' in pollResults) || !('passed' in pollResults)) {
            console.warn("pollResults missing total/passed fields:", pollResults);
        }
